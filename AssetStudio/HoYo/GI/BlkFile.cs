...
namespace AssetStudio
{
    public class BlkFile
    {
        ...
        public BlkFile(FileReader reader, long[] BundlePos=null)
        {
            ...
            var data = reader.ReadBytes((int)(reader.BaseStream.Length - reader.Position));
            ...
            using (var subReader = new EndianBinaryReader(ms, reader.Endian))
            {
                ...
                try
                {
                    ...
                    {
                        for (int i = 0; i < BundlePos.Length; i++)
                        {
                            pos = BundlePos[i];
